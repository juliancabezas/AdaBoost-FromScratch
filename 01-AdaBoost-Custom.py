...
# Custom implementation of the AdaBoost algorithm
...
def main():
    ...
    # Read the .csv breast cancer database using pandas
    data = pd.read_csv("./data/wdbc_data.csv", header = None)

    # Drop the ID column
    data = data.drop(data.columns[0], axis=1)

    # Recode the output column to get -1 (Benign cancer) and 1 (Malign cancer) output values
    ...
    if not os.path.exists('results/n_learners_adaboost_custom.csv'):
        ...
        for n_learners in n_learners_array:
            ...
            adaboost = Adaboost(n_learners = n_learners)
        ...
        df_results.to_csv('results/n_learners_adaboost_custom.csv')
        print("Testing of different number of learners ready!")
    else:
        # In case the parameters were already tuned, read the results
        df_results = pd.read_csv('results/n_learners_adaboost_custom.csv')
